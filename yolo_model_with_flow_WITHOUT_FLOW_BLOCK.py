...
## Input is a List of frames
    def forward(self, frame1_frame2_list ):
        ...
        ## Concatenating NORMALIZED_FLOW rather than FLOW
    ...
    def _create_conv_layers_for_conc_frames(self ):
        ...
        in_channels = self.in_channels * (self.num_frames + 1)
...
if __name__ == "__main__":
    ...
    x = [ norm(i) for i in x ] 
    #print("Torch max::",(torch.max(x[0]), torch.min(x[1]) ))
    print(count_parameters(model))     ## 185302658 parameters requiring grad
    print(count_all_parameters(model))  ## 185302658 total parameters
    out = model(x)
