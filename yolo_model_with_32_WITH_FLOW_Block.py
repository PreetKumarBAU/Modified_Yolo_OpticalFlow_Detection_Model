    ...
    def forward(self, frame1_frame2_list ):
        ...
        for i, layer in enumerate(self.layers_both_frames):
            ...
            if isinstance(layer, ConcatenateModule):
                x = torch.concat([x , frame1_extract_features[0], frame2_extract_features[0] , flow_features_extractation[0] ], dim =1  )
                frame1_extract_features.pop(0)
                frame2_extract_features.pop(0)
                flow_features_extractation.pop(0)
...
if __name__ == "__main__":
    ...
    x = [ norm(i) for i in x ] 
    #print("Torch max::",(torch.max(x[0]), torch.min(x[1]) ))
    print(count_parameters(model))     ## 186649026  parameters requiring grad  
    print(count_all_parameters(model))  ## 186649026 total parameters           
    out = model(x)
